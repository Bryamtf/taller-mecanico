...
const crearVehiculo = async (req, res) => {
    try {
        ...
        if (error.code === 'ER_DUP_ENTRY') {
            return res.status(400).json({ message: "La placa ingresada ya está registrada en el sistema." });
        }
...
const obtenerVehiculos = async (req, res) => {
    try { res.json(await Vehiculo.obtenerTodos()); } 
    catch (error) { res.status(500).json({ message: "Error al obtener vehículos." }); }
};
...
const obtenerVehiculo = async (req, res) => {
    try {
        ...
        res.json(vehiculo);
    } catch (error) { res.status(500).json({ message: "Error al obtener el vehículo." }); }
};
...
const eliminarVehiculo = async (req, res) => {
    try {
        ...
        if (error.code === 'ER_ROW_IS_REFERENCED_2') {
            return res.status(400).json({ message: "No puedes eliminar este vehículo porque tiene citas o cotizaciones asociadas." });
        }
